...
export class ClaudeBin {
  ...
  private static _resolveSystem(configured: string): ResolvedBin | null {
    if (process.platform !== 'win32') {
      ...
      // 服务进程的 PATH 往往比登录 shell 窄（systemd/宝塔起的服务没走 /etc/profile.d），
      // 例如 codex 装在 /www/server/nodejs/vX/bin 就只在登录 PATH 里 → 这里再用登录 shell 兜底解析绝对路径。
      if (!path.isAbsolute(bin) && !this._existsOnPath(bin)) {
        const abs = this._loginShellWhich(bin);
        return abs ? { bin: abs, prefixArgs: [], useShell: false } : null;
      }
  ...
  // 用登录 shell（读 /etc/profile、profile.d）解析命令绝对路径，兜底服务进程 PATH 过窄的情况
  private static _loginShellWhich(bin: string): string | null {
    if (process.platform === 'win32') return null;
    try {
      const out = execSync(`bash -lc 'command -v ${bin}'`, {
        encoding: 'utf8',
        windowsHide: true,
