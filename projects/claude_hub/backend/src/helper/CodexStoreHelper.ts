...
export class CodexStoreHelper {
  ...
  // homeDir：调用方按「真正跑 codex 的用户身份」传入的家目录覆盖值（如 RunAsUser 切了用户）。
  // 不传就退回当前进程的 home —— 服务跑在 root、codex 跑在 claudeuser 时，两边的 ~/.codex 不是同一个，
  // 会导致历史会话读不到、config.toml 写了也不生效。
  static codexHome(homeDir?: string): string {
    return process.env.CODEX_HOME || path.join(homeDir || os.homedir(), '.codex');
  }

  static sessionsRoot(homeDir?: string): string {
    return path.join(this.codexHome(homeDir), 'sessions');
  }
  ...
  static index(homeDir?: string): CodexSessionFile[] {
    const files: string[] = [];
    this._walkFiles(this.sessionsRoot(homeDir), files);
  ...
  static listByCwd(absPath: string, homeDir?: string): CodexSessionFile[] {
    const target = this._norm(absPath);
    return this.index(homeDir).filter((f) => this._norm(f.cwd) === target);
  }

  // 按 uuid 定位文件（全量索引里查）
  static findFile(uuid: string, homeDir?: string): CodexSessionFile | null {
    if (!uuid) return null;
    return this.index(homeDir).find((f) => f.sessionId === uuid) || null;
  }

  // 该会话是否存在（用于决定能否 resume）
  static sessionExists(uuid: string, homeDir?: string): boolean {
    return !!this.findFile(uuid, homeDir);
  }
  ...
  static removeSessionFile(uuid: string, homeDir?: string): void {
    const f = this.findFile(uuid, homeDir);
    if (!f) return;
    try {
