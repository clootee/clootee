...
export class SessionManager extends SessionManagerStruct {
  ...
  protected static _codexMetas(rootId: string): Session[] {
    try {
      const root = RootManager.getRoot(rootId);
      return CodexSession.metas(root.path, rootId, Settings.effectiveHomeDir());
    } catch {
      return [];
    }
  }
  ...
  protected static _buildMeta(id: string): Session | null {
    ...
    try {
      return CodexSession.metaOne(rootId, uuid, Settings.effectiveHomeDir());
    } catch {
      return null;
    }
  }
  ...
  protected static _loadMessages(session: Session): Message[] {
    ...
    try {
      if (session.engine === 'codex') {
        return CodexSession.importMessages(session.claudeSessionId, Settings.effectiveHomeDir());
      }
  ...
  private static _sessionFilePath(session: Session): string | null {
    ...
    try {
      if (session.engine === 'codex') {
        return CodexStoreHelper.findFile(session.claudeSessionId, Settings.effectiveHomeDir())?.file || null;
      }
      const root = RootManager.getRoot(session.rootId);
  ...
  protected static _removeJsonl(id: string): void {
    ...
    try {
      if (engine === 'codex') {
        CodexStoreHelper.removeSessionFile(realUuid, Settings.effectiveHomeDir());
        return;
      }
      if (engine === 'claude') {
        const root = RootManager.getRoot(rootId);
