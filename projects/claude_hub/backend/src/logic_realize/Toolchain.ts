...
export class Toolchain extends ToolchainStruct {
  ...
  private static _whichIn(name: string, excludeOutEnd: boolean): string | null {
    ...
    // 服务进程 PATH 往往比登录 shell 窄（宝塔/systemd 起的服务不读 /etc/profile.d），
    // 例如 codex 只装在 /www/server/nodejs/vX/bin → which 找不到、界面显示"未安装"。用登录 shell 再兜一次。
    if (!out.trim() && process.platform !== 'win32') {
      try {
        out = String(
